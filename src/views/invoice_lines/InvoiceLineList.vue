		...
		<div v-animate class="p-5 border rounded-md shadow-sm" id="div_table">
			<div class="flex flex-col sm:flex-row sm:items-end xl:items-start">
				...
				<div class="flex mt-5 sm:mt-0">
					<button class="btn-primary w-1/2 sm:w-auto mr-2" @click.prevent="showCreateInvoiceLine">
...
<script setup>
	...
	// On reset filter
	const onResetFilter = () => {
		filter.field = "invoice_header_id";
		filter.type = "like";
		filter.value = "";
		onFilter();
	};
		...
		tabulator.value = new Tabulator(table.value, {
			...
			columns: [
				...
				},
				{
					...
					cellClick: (e, cell) => {
						showEditInvoiceLine(cell.getData().id);
						e.preventDefault();
					},
				},
				{
					...
					cellClick: (e, cell) => {
						//deleteInvoiceLine(cell.getData().id, cell.getData().name);
						showDeleteInvoiceLine(cell.getData().id);
						e.preventDefault();
					},
				},
			],
