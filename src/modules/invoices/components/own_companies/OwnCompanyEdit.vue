...
const { ownCompany, getOwnCompany } = useOwnCompanies();
const { t } = useI18n();
const props = defineProps(['ownCompanyId']);
const emit = defineEmits(['cancelEdit', 'updateOwnCompanyForm']);
...
const save = () => {
	...
	if (validate.value.$invalid) {
		...
		emit('updateOwnCompanyForm', ownCompany.value.id, formData);
	}
};

onMounted(async () => {
	await getOwnCompany(props.ownCompanyId);
	formData.country_id = ownCompany.value.country_id;
	formData.name = ownCompany.value.name;
	formData.tax = ownCompany.value.tax;
	formData.address = ownCompany.value.address;
	formData.state = ownCompany.value.state;
	formData.municipality = ownCompany.value.municipality;
	formData.email = ownCompany.value.email;
	formData.phone = ownCompany.value.phone;
	formData.zip_code = ownCompany.value.zip_code;
	formData.website = ownCompany.value.website;
	await getCountries();
});
